...
import AssignQCTable from "./AssignQCTable";
import { getQcList } from "../../config/services/implementationForm";
...
const AssignQCList = () => {
  ...
  const fetchSiteSurveyListing = async () => {
    ...
    try {
      ...
      response = await getQcList(queryObj);
    ...
    >
      <div className="tableCardContainer">
        <div className="crm-sd-claims-header">
          <Typography variant="h4" component="h4" sx={{ margin: "20px" }}>
            Quality Check List
          </Typography>
        </div>
        ...
        {!isLoading ? (
          siteSurveyList?.length ? (
            <>
              <AssignQCTable siteSurveyList={siteSurveyList} />
            </>
          ) : (
            <>
