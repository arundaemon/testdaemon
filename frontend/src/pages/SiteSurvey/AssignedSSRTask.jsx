...
import { fieldTab, siteSurveyConstant } from "../../constants/general";
import { DecryptData } from "../../utils/encryptDecrypt";
import { getLoggedInRole } from "../../utils/utils";
import { getAssignedTaskList } from "../../config/services/implementationForm";
import { getUserData } from "../../helper/randomFunction/localStorage";
import SiteSurveyTaskTable from "./AssignedTaskTable";

const SiteSurveyTask = () => {
  ...
  const getSSRActivity = async () => {
    ...
    let params = {
      type:fieldTab?.SSR,
      assignedEngineerRoleName: getUserData('userData')?.crm_role,
    ...
    setLastPage(false);
    try {
      let res = await getAssignedTaskList(params);
      ...
      >
        <Grid className={classes.cusCard}>
          <Grid item md={12} xs={12} py={4}>
            {/* <Typography className={classes.heading}>Site Survey Tasks</Typography> */}
            <TextField
          ...
          {!loader ? (
            userActivity?.length > 0 ? (
              <Container className="table_max_width px-0">
                <SiteSurveyTaskTable data={userActivity} />
