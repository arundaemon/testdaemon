...
import { fetchImplementationList, getImplementationListWithEr } from "../../config/services/implementationForm";
...
import { QuoteType, assignedEngineerType, fieldTab } from "../../constants/general";
const style = {
  position: "absolute",
  top: "50%",
  left: "50%",
...
const HardwareQualityList = () => {
  ...
  const getImplementationList = async (childRoleNames) => {
    try {
      ...
      let params = {
        pageNo: pageNo - 1,
        count: itemsPerPage,
        search: QuoteType?.isSSRSUBMITTED,
        childRoleNames,
      };
      setLastPage(false);
      setIsLoading(true);
      ...
      let res = await getImplementationListWithEr(params);
  ...
  const productInterstList = () => {
    return implementationNo?.map((impNumber) => {
      let data = implementationData?.find(
        (obj) => obj?.impFormNumber === impNumber
      );
      return data?.productDetails?.map((pd) => {
        return {
          leadId: pd?.leadId ?? "",
          schoolId: data?.schoolId,
          name: pd?.productName,
          learningProfile: pd?.productName,
          learningProfileCode: pd?.productRefCode,
          learningProfileGroupCode: pd?.groupCode,
          learningProfileRefId: pd?.learningProfileRefId ?? "",
          learningProfileGroupName: pd?.groupName,
        };
      });
    });
  };
  ...
  const getAssignData = () => {
    if (selectedUser?.length > 0)
      return implementationNo?.map((impNumber) => {
        return selectedUser?.map((obj) => {
          return {
            impFormNumber: impNumber,
            assignedEngineerName: obj?.displayName?.toString(),
            assignedEngineerEmpCode: obj?.userName?.toString(),
            assignedEngineerRoleName: obj?.roleName?.toString(),
            assignedEngineerProfileName: obj?.profileName?.toString(),
          };
        });
      });
  };
  ...
  const getCustom = () => {
    ...
    getAssignData()
      ?.flat()
      ?.map((obj) => {
        impFormNumber = obj?.impFormNumber;
        productTitle = implementationData
          ?.find((obj) => obj?.impFormNumber === impFormNumber)
          ?.productDetails?.map((obj) => obj?.productName);

        uniqueSet = new Set(productTitle);
        uniqueProductArray = Array.from(uniqueSet);

        return {
          title: uniqueProductArray?.map((obj) => obj)?.join(", "),
          description: `This implementation (${impFormNumber}) has  Assigned to ${obj?.assignedEngineerName?.toString()} `,
          redirectLink: `/authorised/site-survey/${impFormNumber}`,
          empCode: [obj?.assignedEngineerEmpCode],
  ...
  const assignEngineer = async () => {
    ...
    try {
      let params = {
        implementationList: getAssignData()?.flat(),
        ...
        type: assignedEngineerType?.QC,
        status: assignedEngineerType?.isQCEngineerStatus,
        qcEngineerAssigned: true,
        stage: 'H/W Implementation'
        // productList: productInterstList()?.flat(),
      };

      notifyParams = getAssignData()
        ?.flat()
        ?.map((obj) => {
          impFormNumber = obj?.impFormNumber;
          productTitle = implementationData
          ...
          return {
            title: uniqueProductArray?.map((obj) => obj)?.join(", "),
            description: `This implementation (${impFormNumber}) has  Assigned to ${obj?.assignedEngineerName?.toString()} `,
            redirectLink: `/authorised/QC-detail/${impFormNumber}`,
            empCode: [obj?.assignedEngineerEmpCode],
            notificationDate: moment().format("YYYY-MM-DD"),
          };
        });
    ...
    >
      <div className="tableCardContainer">
        ...
        <div className={classes.btnApproval}>
          {implementationNo.length > 0 && rolesList?.length > 0  && (
            <Button
          ...
          implementationData.length ? (
            <>
              <ImplementationTableList
                ...
                implementationType={fieldTab?.isQC}
...
export default HardwareQualityList;
