...
import CreateCustomerResponse from '../components/activityForm/CreateCustomerResponse'
import { getAllCustomerResponses, deleteCustomerResponse} from '../config/services/customerResponse'
import CustomerResponseTable from '../components/activityForm/CustomerResponseTable';
...
const CustomerResManagement = () => {
    ...
    const fetchSubjectList = async (scrollFlag = false) => {
        ...
        getAllCustomerResponses(params)
            .then((res) => {
                //   console.log(res, '...res')
                 setCycleTotalCount(res?.count)
                 setSubjectList(res?.customerResponseList);
                 if (scrollFlag) {
    ...
    const deleteJourneyOne = (params) => {
        ...
        setDeleteObj({ customerResponseId: params?._id, customerResponse: params?.customerResponse})
        setDeletePopup(true)
    ...
    const updateJourneyOne = (params) => {
        setCreateSubject(!createSubject)
        setUpdateObj({ customerResponseId: params?._id, customerResponse: params?.customerResponse})
    ...
    const submitDeleteJourney = () => {
        let { customerResponseId } = deleteObj
        deleteCustomerResponse({ customerResponseId })
            .then(res => {
    ...
    return (
        <>
            <Page title="Extramarks | Customer Response Management" className="main-container compaignManagenentPage datasets_container">
                <div>

                    <div>
                        {createSubject &&
                            <div className='createCampaign' >
                                <CreateCustomerResponse fetchSubjectList={fetchSubjectList} showCreateSubject={showCreateSubject} updateObj={updateObj} />
                            </div>
                        }
                    ...
                    <div className='tableCardContainer' >
                        <div ref={divRef}>
                            ...
                            <TextField className={`inputRounded search-input`} type="search"
                                placeholder="Search By Customer Response"
                            ...
                            <CustomerResponseTable list={subjectList} pageNo={pageNo} itemsPerPage={itemsPerPage} search={search} deleteJourneyOne={deleteJourneyOne} updateJourneyOne={updateJourneyOne}/>
                    ...
                    <Box className={classes.modalPaper + " modal-box modal-md"} id="transition-modal-title">
                        <Box className="modal-header p-1" >
                            <Typography variant="subtitle1" className={classes.modalTitle + " modal-header-title"} >
                                {`Are you sure to delete "${deleteObj?.customerResponse}" ?`}
                            </Typography>
                        </Box>
...
export default CustomerResManagement
