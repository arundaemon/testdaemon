...
import CreateReasonForDisqualifiction from '../components/activityForm/CreateReasonForDisqualification'
import { getReasonForDisqualificationList, deleteReasonForDisqualification} from '../config/services/reasonForDisqualification'
import ReasonForDisqualificationTable from '../components/activityForm/ReasonForDisqualificationTable';
...
const ReasonForDisqualificationManagement = () => {
    ...
    const fetchSubjectList = async (scrollFlag = false) => {
        ...
        getReasonForDisqualificationList(params)
            .then((res) => {
                //   console.log(res, '...res')
                setCycleTotalCount(res?.count)
                setSubjectList(res?.responseList);
                if (scrollFlag) {
    ...
    const deleteJourneyOne = (params) => {
        ...
        setDeleteObj({ reasonForDisqualificationId: params?._id, reasonForDisqualification: params?.reasonForDisqualification})
        setDeletePopup(true)
    ...
    const updateJourneyOne = (params) => {
        setCreateSubject(!createSubject)
        setUpdateObj({ reasonForDisqualificationId: params?._id, reasonForDisqualification: params?.reasonForDisqualification})
    ...
    const submitDeleteJourney = () => {
        let { reasonForDisqualificationId } = deleteObj
        deleteReasonForDisqualification({ reasonForDisqualificationId })
            .then(res => {
                    ...
                    <div>
                        {createSubject &&
                            <div className='createCampaign' >
                                <CreateReasonForDisqualifiction fetchSubjectList={fetchSubjectList} showCreateSubject={showCreateSubject} updateObj={updateObj} />
                            </div>
                        }
                    ...
                    <div className='tableCardContainer' >
                        <div ref={divRef}>
                            ...
                            <TextField className={`inputRounded search-input`} type="search"
                                placeholder="Search By Reason For Disqualification"
                            ...
                            <ReasonForDisqualificationTable list={subjectList} pageNo={pageNo} itemsPerPage={itemsPerPage} search={search} deleteJourneyOne={deleteJourneyOne} updateJourneyOne={updateJourneyOne}/>
                    ...
                    <Box className={classes.modalPaper + " modal-box modal-md"} id="transition-modal-title">
                        <Box className="modal-header p-1" >
                            <Typography variant="subtitle1" className={classes.modalTitle + " modal-header-title"} >
                                {`Are you sure to delete "${deleteObj?.reasonForDisqualification}" ?`}
                            </Typography>
                        </Box>
...
export default ReasonForDisqualificationManagement
