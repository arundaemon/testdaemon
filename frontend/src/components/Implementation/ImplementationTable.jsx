...
export const ImplementationTableList = ({
  data,
  getSelectedItem,
  disabledCheckBox,
  implementationType,
  ...
  const getEngineerData = (params) => {
    let { assigned_engineer_info } = params;
    let list = [];
    assigned_engineer_info?.filter((item) => {
      if (item?.type === implementationType && item?.assignedEngineerName) {
        list.push(item?.assignedEngineerName);
      }
    });
        ...
        >
          <TableHead>
            <TableRow className="cm_table_head">
              {!disabledCheckBox && <TableCell></TableCell>}
              <TableCell>
                {/* {" "} */}
            ...
            {data?.map((item) => (
              ...
              >
                {!disabledCheckBox && (
                  <TableCell>
                    <Checkbox
                      checked={implementationNo.includes(item?.impFormNumber)}
                      onChange={(event) =>
                        handleCheckboxChange(event, item?.impFormNumber)
                      }
                    />
                  </TableCell>
                )}
                ...
                <TableCell>
                  {/* {item?.productDetails?.length
                    ? item?.productDetails
                      ...
                      ?.join(",")
                    : "NA"} */}
                  {item?.productDetails[0]?.groupCode?.toUpperCase()?.replace('_', ' ')}
                </TableCell>
                <TableCell>
                ...
                <TableCell>{getEngineerData(item)}</TableCell>
                {/* <TableCell>
