...
const EditAddendumDetails = ({ rowData, isApprovalFlow }) => {

    let addendumID = rowData?.referenceCode?.slice(4)
    const location = useLocation()
    const addendum_auto_id = !isApprovalFlow ? location?.state?.addendum_auto_id : addendumID
    ...
    const getAddendumDetail = async (id) => {
        let detail = await listAddendumDetail({ uuid, addendum_auto_id: id })
        detail = detail?.data?.addendum_details[0]
        setAddendumDetails(detail)
        setCollectionDetails(detail?.collection_details || rowData?.collectionDetails)
    ...
    useEffect(async () => {
        if (addendum_auto_id) {
            await getAddendumDetail(addendum_auto_id)
        }
    }, [])

    useEffect(() => {
        if (!location?.state?.addendum_auto_id) {
            let collectionData = []
            ...
            collectionData?.map((obj) => {
                if (!obj?.collection_amount || obj?.collection_date === 'Invalid Date') {
                    isBool = false
                }
            })
    ...
    return (
        <Box>
                                                ...
                                                <TableCell align="left" sx={{ width: "20%" }}>
                                                    <Box sx={styles.productSec}>
                                                        <TextField value={obj?.collection_amount} onChange={(e) => {
                                                            ...
                                                            if (addendumDetails?.total_due_amount < Number(e.target.value) + Number(sum)) {
                                                    ...
                                                    <Box sx={{ display: "flex" }}>
                                                        ...
                                                        }} />
                                                        {
                                                            ...
                                                            <AddIcon sx={{ color: "#F45E29", border: "2px solid #F45E29", cursor: "pointer", borderRadius: "4px", marginLeft: "20px" }} onClick={() => {
                                                                ...
                                                                if (addendumDetails?.total_due_amount <= Number(sum)) {
            ...
            <Box sx={{ margin: "20px", display: "flex", flexDirection: "row-reverse" }}>
                ...
                {location?.state?.addendum_auto_id && <Button variant='outlined' sx={{
                    ...
                    onClick={async () => {
                        ...
                        let submitData = {
                            school_code: addendumDetails?.school_code,
                            uuid: uuid,
                            po_code: addendumDetails?.po_code,
                            total_due_amount: addendumDetails?.total_due_amount,
                            collection_details: collectionData,
                            addendum_proof_file: addendumDetails?.addendum_proof_file,
                            approval_status_id: 1,
                            addendum_comment: addendumDetails?.addendum_comment,
                            reason_id: addendumDetails?.reason_id,
                            addendum_auto_id: addendum_auto_id,
                            modified_by: getUserData("userData")?.name,
...
export default EditAddendumDetails