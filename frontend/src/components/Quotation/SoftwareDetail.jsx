...
export const SoftwareDetail = ({
  ...
  const getCustomOption = (key, rowIndex, isPackageOption) => {
    ...
    if (key === QuotaionFormFields["field5"]["label"]) {
      ...
      var selectedPackage = newFields[rowIndex]?.dependentFields?.find(
        (obj) => obj.fieldName === QuotaionFormFields["field3"]["fieldName"]
      )?.value;
      ...
      if (isAddedPackage?.length && selectedPackage) {
        var hasDuplicatePackage = isValueDuplicated(
          isAddedPackage,
    ...
    let isExistOption = [];
    if (
      key === QuotaionFormFields["field5"]["label"] &&
      hasDuplicateValue &&
      hasDuplicatePackage
    ) {
      ...
      isExistOption = isExistOption
        ...
        ?.filter((obj) => obj?.packageID === selectedPackage?.value)
        ?.map((obj) => obj?.label);
  ...
  const renderTextField = (key, obj, rowIndex, fieldIndex) => {
    return (
      <input
        className={
          `crm-form-input dark ` + !obj?.isReadOnly
            ? !isFieldValid || obj?.value
              ? classes.inputStyle
              : classes.inputBorderStyle + ` Mui-error `
            : classes.inputStyle
        }
        name="name"
        type="text"
      ...
      <>
        <input
          ...
          onChange={(e) => {
            ...
            // if (e.target.value) {
            //   if (!(e.target.value >= 1)) {
            //     var currentField = newFields[rowIndex].dependentFields?.find(
            //       (obj) => obj.fieldName === currentObj?.fieldName
            //     );
            //     currentField.value = 1;
            //   }
            // }

            if (obj?.fieldName === QuotaionFormFields["field2"]["fieldName"]) {
              if (e.target.value >= 1)
        ...
        {obj?.fieldName === QuotaionFormFields["field2"]["fieldName"] &&
          newFields[rowIndex]?.message && (
            <span className="crm-contract-formitem-error">
              {newFields[rowIndex]?.message}
            </span>
          )}
      </>
        ...
        <div>
          ...
          >
            ...
            >
              <TableHead>
                <TableRow>
                  {softwareTableData?.dependentFields?.map(
                    (col, index) =>
                      !(
                        ...
                        <TableCell
                          ...
                          // className={
                          //   softwareTableData?.dependentFields[index]
                          //     .fieldType === "Integer"
                          //     ? "width-100px"
                          //     : `width-120px`
                          // }
