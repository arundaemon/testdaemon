...
import { getCampaignName } from "../config/services/packageBundle";
...
export default function MatrixCampaignListing() {
  ...
  const fetchSourceList = async () => {
    ...
    let obj = {
      ...
      search_by: searchBy.campaignName,
      search_val: search,
      status: [1, 2],
      order_by: "updated_on",
      order: "DESC"
    ...
    try {
      const response = await getCampaignName(obj);
      setLastPage(false)
      setSourceList(response?.data?.campaign_list)
      if (response?.data?.campaign_list?.length < itemsPerPage) setLastPage(true)
      setLoading(false)
    }
    catch (err) {
      console.log("error in getCampaignList: ", err);
      toast.error("***Error***");
    }
      ...
      >
        <div className="mainContainer">
          ...
          >
            ...
            <div className="right">
              <Link to={"/authorised/matrix-campaign"}>
