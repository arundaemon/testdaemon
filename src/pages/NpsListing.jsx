...
const NPSListing = () => {
  ...
  const npsRequestStatusMap={
    1: "Pending",
    2: "Approved By Collection Head",
    3: "Rejected By Collection Head",
    4: "Modified By Admin",
    5: "Rejected By Admin",
  }
  const statusMap ={
  ...
  const fetchNpsListing = () => {
    let params = {
      uuid: getUserData("loginData")?.uuid,
      status: [1],
      data_source: "invoice",
      request_type: "NPS",
      schedule_data_type: ["collection","invoice"],
      schedule_for: ["SW",'SSP'],
        ...
        <Box className="crm-page-listing">
          <div className="crm-page-schedule-header">
            <div className="crm-page-schedule-header-label">
              ...
              <div className="left">
                <h3>Collection & Invoice Schedule List</h3>
              </div>
              {
                !isMobile
                  ? <div className="right">
                      ...
                      <TableBody>
                        {scheduleList.map((scheduleObj, index) => (
                          <TableRow key={`schedule-${index}`}>
                            ...
                            <TableCell>{scheduleObj.product_group_key ?? "NA"}</TableCell>
                            <TableCell>{scheduleObj.implementation_form_id}</TableCell>
                            <TableCell>{statusMap[scheduleObj.status]}</TableCell>
                            <TableCell>{npsRequestStatusMap[scheduleObj?.nps_details?.nps_request_status] ?? "NA"}</TableCell>
                            <TableCell>
                              {moment(scheduleObj.created_on).format("DD MMM, YYYY")}
                            </TableCell>
                            <TableCell>
                              {moment(scheduleObj.updated_on).format("DD MMM, YYYY")}
                            </TableCell>
                            <TableCell>
                              <Box>
                                <Box>
                      ...
                      <Box className="crm-g-schedule-listitem" key={index}>
                        <Grid container spacing={2.5}>
                          ...
                          <Grid item xs={6}>
                            ...
                            <Typography component={"p"}>{npsRequestStatusMap[scheduleObj?.nps_details?.nps_request_status] ?? "NA"}</Typography>
                          </Grid>
                          <Grid item xs={6}>
...
export default memo(NPSListing);
