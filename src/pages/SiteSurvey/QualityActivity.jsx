...
const QualityActivityTable = ({ data }) => {
  const navigate = useNavigate();

  const isRedirect = (obj) => {
    let implementationCode = obj?._id;
    navigate(`/authorised/add-QC/${implementationCode}`, {
      state: {
              ...
              ? data?.map((obj, index) => {
                  return (
                    ...
                    >
                      ...
                      <TableCell>
                        {" "}
                        {obj?.documents?.[0]?.createdAt
                          ? moment(obj?.documents?.[0]?.createdAt).format(
                              "DD-MM-YYYY"
                            )
                          : "NA"}
                      </TableCell>
...
export default QualityActivityTable;
