...
import { createProject, updateProject } from '../../config/services/project';
import { getRuleList, deleteRule } from '../../config/services/rules';
import SearchIcon from '../../assets/icons/icon_search.svg';
import _ from 'lodash';
import { ProjectPopup } from '../../components/projectManagement';
import { RuleTable } from '../../components/ruleManagement';
...
export default function ProjectManagement() {
    ...
    const addOrEdit = () => {
        if (validateAddProject(recordForEdit)) {
            ...
            if (paramsObj?._id) {
                paramsObj.ruleId = paramsObj?._id
    ...
    const deleteRuleF = params => {
        setDeleteObj({ ruleId: params?._id, ruleName: params?.ruleName })
        setDeletePopup(true)
    }

    const submitDeleteRule = () => {
        let { ruleId } = deleteObj
        deleteRule({ ruleId })
            .then(res => {
                if (res?.result) {
                    handleCancelDelete()
    ...
    const handleEditRule = (ruleId) => {
        navigate('/authorised/rule-management/update-rule/' + ruleId)
    }
    ...
    useEffect(() => fetchRulesList(), [search, sortObj, pageNo, itemsPerPage]);
        ...
        <>
            <Page title="Extramarks | User Management" className="main-container datasets_container">
                <Container className='table_max_width'>
                            ...
                            <Grid container justifyContent="flex-end" spacing={2.5}>
                                <Grid item xs={6} sm={6} md={6} lg={3} display="flex" justifyContent="flex-end" >
                                    <Controls.Button
                                        ...
                                        onClick={() => navigate('/authorised/rule-management/create-rule')}
                                    />
                                </Grid>
                            </Grid>
                        </Grid>
                    ...
                    {loader && <Loader />}
                    {userTypeList?.length ? <RuleTable handleEditRule={handleEditRule} list={userTypeList} openInPopup={openInPopup} deleteUserType={deleteRuleF} pageNo={pageNo} itemsPerPage={itemsPerPage} handleSort={handleSort} sortObj={sortObj} /> :
                    ...
                    <Box className={classes.modalPaper + " modal-box modal-md"} id="transition-modal-title">
                        <Box className="modal-header p-1" >
                            <Typography variant="subtitle1" className={classes.modalTitle + " modal-header-title"} >
                                {`Are you sure to delete "${deleteObj?.ruleName}" ?`}
                            </Typography>
                        </Box>
                        ...
                        <Box className="modal-footer text-right" >
                            ...
                            <Button onClick={submitDeleteRule} color="primary" autoFocus className={" report_form_ui_btn submit"} variant="contained"> Submit </Button>
                        </Box>
                        {/* </Box> */}
                    </Box>
                </Fade>
